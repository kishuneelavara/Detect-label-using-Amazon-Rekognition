...
from utils.detect_label_aws import get_label
...
def detect():
    ...
    response,res_img,res_response,statement = get_label(filename,label)
    if statement=="success":
        return render_template('index.html', jsonfile=json.dumps(response, indent=4),result_json_response=res_response, filename=res_img, result_image=res_img, statement=statement)
    else:
        return render_template('index.html',statement=statement)
